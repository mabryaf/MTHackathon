# -*- coding: utf-8 -*-
"""SmartQueue v0.3.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/18boXwx1XjsQi6mWUh4iMgQIsIpl8u1XS
"""
...
class Reservation:
  def __init__(self, person_id, reward_points):
    self.id = uuid.uuid1()
...
class TestReservation(unittest.TestCase):
  def setUp(self):
    person_id = "abc123"
    reward_points = 3
    self.reservation = Reservation(person_id, reward_points)
...
class Queue:
  ...
  def __remaining_capacity(self, remaining_resource_capacity, remaining_location_capacity):
  ...
  def reward(self, remaining_resource_capacity, remaining_location_capacity):
    remaining_capacity = self.__remaining_capacity(remaining_location_capacity, remaining_resource_capacity)
  ...
  @staticmethod
  def __reservation_is_active (reservation):
    active_states = [ReservationState.RESERVED, ReservationState.COMPLETED]
    reservation_active = reservation.state in active_states
  ...
  def reserve(self, person_id, reward_points, remaining_resource_capacity, remaining_location_capacity):
    remaining_capacity = self.__remaining_capacity(remaining_location_capacity, remaining_resource_capacity)
    capacity_remains = remaining_capacity > 0

    result = ReserveActionResult.FAILURE
    reservation_is_new = self.__find_reservation(person_id) is None

    if reservation_is_new and capacity_remains:
      res = Reservation(person_id, reward_points)
      self.reservations.append(res)
      result = ReserveActionResult.SUCCESS
...
class TestQueue(unittest.TestCase):
  def setUp(self):
    ...
    self.queue = Queue(5, open, close, address, resource_id)
  ...
  def test_queue_creation(self):
    ...
    self.assertEqual(self.queue.max_capacity, 5)
  ...
  def test_making_reservations(self):
    ...
    result = self.queue.reserve("abc123", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.assertEqual(result, ReserveActionResult.SUCCESS)
    self.assertEqual(self.queue.active_occupants(), 1)

    #the queue should ignore duplicate reservation requests
    result = self.queue.reserve("abc123", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.assertEqual(result, ReserveActionResult.FAILURE)
    self.assertEqual(self.queue.active_occupants(), 1)

    #the queue should honor new reservation requests
    self.queue.reserve("xyz456", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.queue.reserve("ijk789", reward_points, remaining_resource_capacity, remaining_location_capacity)
  ...
  def test_canceling_reservations(self):
    ...
    self.queue.reserve("abc123", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.queue.reserve("xyz456", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.queue.reserve("ijk789", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.assertEqual(self.queue.active_occupants(), 3)
  ...
  def test_missing_reservations(self):
    ...
    self.queue.reserve("xyz456", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.queue.reserve("ijk789", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.queue.reserve("def321", reward_points, remaining_resource_capacity, remaining_location_capacity)
  ...
  def test_queue_capacity(self):
    ...
    self.queue.reserve("person1", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.queue.reserve("person2", reward_points, remaining_resource_capacity, remaining_location_capacity)

    remaining_resource_capacity = 0
    remaining_location_capacity = 3
    self.queue.reserve("person3", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.assertEqual(self.queue.active_occupants(), 2)
    ...
    self.queue.reserve("person4", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.assertEqual(self.queue.active_occupants(), 3)
    ...
    remaining_location_capacity = 5
    self.queue.reserve("person4", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.queue.reserve("person5", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.queue.reserve("person6", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.assertEqual(self.queue.active_occupants(), 5)
  ...
  def test_reward_points_calculation_of_a_queue(self):
    ...
    self.queue.reserve("person1", stubbed_reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.queue.reserve("person2", stubbed_reward_points, remaining_resource_capacity, remaining_location_capacity)
    middle_reward = self.queue.reward(remaining_resource_capacity, remaining_location_capacity)
    self.assertEqual(middle_reward, 0.6)
    ...
    self.assertEqual(little_reward, 0.4)
...
class TestLocation(unittest.TestCase):
  ...
  def test_location_capacity(self):
    ...
    self.location.queues[0].reserve("person1", reward_points, remaining_resource_capacity, remaining_location_capacity)
    ...
    self.location.queues[1].reserve("person2", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.location.queues[1].reserve("person3", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.location.queues[1].reserve("person4", reward_points, remaining_resource_capacity, remaining_location_capacity)
    ...
    self.location.queues[2].reserve("person5", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.location.queues[2].reserve("person6", reward_points, remaining_resource_capacity, remaining_location_capacity)
  ...
  def test_location_capacitys_effect_on_queue_capacity(self):
    ...
    remaining_location_capacity = self.location.remaining_capacity(queue2_open, queue2_close)
    result = self.location.queues[0].reserve("person3", reward_points, remaining_resource_capacity, remaining_location_capacity)
    self.assertEqual(result, ReserveActionResult.FAILURE)
    self.assertEqual(self.location.remaining_capacity(queue2_open, queue2_close),0)
...
class SmartQueue:
  ...
  def update(self, queue_schedule):
    ...
    for resource in queue_schedule:
      ...
      occupant_sensor = resource['occupancy_sensor']
      new_resource = Resource(resource_id, max_occupancy, occupant_sensor)
      self.__add_resource_if_it_does_not_exist(id, new_resource)
  ...
  def reserve(self, person_id, queue_id):
    ...
    reward_points = queue.reward(remaining_resource_capacity, remaining_location_capacity)
    result = queue.reserve(person_id, reward_points, remaining_resource_capacity, remaining_location_capacity)
    return result
...
class TestSmartQueue(unittest.TestCase):
  ...
  def test_reservation_creation(self):
    ...
    result = self.smartqueue.reserve('person1', queue_id)
    self.assertEqual(result, ReserveActionResult.SUCCESS)
